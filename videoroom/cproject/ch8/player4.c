...
void video_refresh_timer(void *userdata){
    ...
    if(is->video_st){
        if(is->pictq_size == 0){
            ...
            vp = &is->pictq[is->pictq_rindex];
            schedule_refresh(is, 40); // resend timer
...
int queue_picture(VideoState *is, AVFrame *pFrame){
    ...
    if(!vp->pict || vp->width != is->video_ctx->width || vp->height != is->video_ctx->height){
        vp->allocated = 0;
        alloc_picture(is);
        if(is->quit){
...
int video_thread(void *arg){
    ...
    for(;;){
        ...
        if(frameFinished){
            if(queue_picture(is, pFrame) < 0){  // 解码后放入解码后的队列
                break;
            }
        }
...
int decode_thread(void *arg){
    ...
    win = SDL_CreateWindow("Media Player v3.0",
