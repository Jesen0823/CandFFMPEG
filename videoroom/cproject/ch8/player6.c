/*
 *clang -g -o player6 player6.c `pkg-config --cflags --libs sdl2 libavutil libavformat libavcodec libswresample libswscale`
...
int demux_thread(void *arg) {
  ...
  for(i=0; i<pFormatCtx->nb_streams; i++) {
    ...
    if(pFormatCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_AUDIO &&
      ...
      audio_index=i;
      // 打印音频采样
      fprintf(stderr,"audio rate is %d",pFormatCtx->streams[i]->codec->sample_rate);
    }
  }
  if(audio_index >= 0) {
